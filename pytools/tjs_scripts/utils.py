...
def strict_check_model(model_or_path: Union[onnx.ModelProto, str, Path]):
    try:
        ...
        if "No Op registered for" in str(e):
            ...
            # Non-fatal: the checker's full_check is unreliable on large /
            # mixed-precision graphs. Real breakage is caught downstream by
            # the CPU verify step, which actually runs the model.
            logger.warning("model check failed (saving anyway; verify will catch real breakage): %s", e)
